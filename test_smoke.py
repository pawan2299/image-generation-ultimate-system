...
def env(monkeypatch):
    ...
    monkeypatch.setenv("GEMINI_API_KEYS", "gemini-test-1,gemini-test-2") # Multi-key setup
...
def test_config_fallback(env):
    ...
    assert config.Config.GEMINI_API_KEYS == ["gemini-test-1", "gemini-test-2"]
...
def test_config_missing_raises(monkeypatch):
    for key in ["TELEGRAM_BOT_TOKEN", "GEMINI_API_KEYS", "GEMINI_API_KEY", "GROQ_API_KEY", "DATABASE_URL", "WEBHOOK_URL", "RENDER_EXTERNAL_URL", "WEBHOOK_PATH", "PORT"]:
